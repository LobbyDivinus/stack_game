...
use stm32f7::{system_clock, board, embedded, lcd, sdram, touch, i2c};
...
impl Renderer {
    ...
    fn set_pixel(&mut self, x: i32, y: i32, color: lcd::Color) {
        ...
        if self.direct {
            ...
            self.layer.print_point_color_at(pixel.x as usize, pixel.y as usize, color);
    ...
    fn clear_area(&mut self, x: i32, y: i32, w: i32, h: i32) {
        ...
        for py in x..x + w {
            for px in y..y + h {
                ...
                self.layer.print_point_color_at(px as usize, py as usize, color);
            }
        }
    }
...
fn main(hw: board::Hardware) -> ! {
    ...
    let mut gpio = Gpio::new(gpio_a, gpio_b, gpio_c, gpio_d,
                             gpio_e, gpio_f, gpio_g, gpio_h,
                             gpio_i, gpio_j, gpio_k);
...
fn game(renderer: &mut Renderer, i2c_3: &mut i2c::I2C) {
    ...
    draw_block(renderer, 0, last_block_start - 1, block_height, last_block_width, block_color);
    ...
    loop {
        ...
        draw_block(renderer, cur_stack_height, block_start, block_height, block_width, block_color);
        ...
        if tapped && !last_tapped {
            ...
            draw_block(renderer, cur_stack_height - 1, block_start, block_height + 1, block_width, block_color);
        ...
        loop {
            ...
            if cur_ms - ms >= ms_per_frame { break; }
        }
...
fn draw_block(d: &mut Renderer, x: i32, y: i32, w: i32, h: i32, color: lcd::Color) {
    ...
    for i in 0..h/15 {
        ...
        draw_line(d, x + w / 2 + 1, y + i * 15 + 5, x + w / 2 + 1, y + i * 15 + 11, color);
        draw_line(d, x + w / 2 - 3, y + i * 15 + 8, x + w / 2 + 5, y + i * 15 + 8, color);
    }
}
